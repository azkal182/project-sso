...
describe('SearchProvider and CommandMenu', () => {
  ...
  it('renders the command palette when the palette is open', async () => {
    ...
    await expect
      .element(screen.getByRole('option', { name: 'System' }))
      .toBeInTheDocument()
  ...
  it('navigates to a top-level route and closes the palette when a nav item is selected', async () => {
    ...
    await userEvent.click(screen.getByText('Applications'))

    expect(mocks.navigate).toHaveBeenCalledWith({ to: '/applications' })
    await expect
      .element(screen.getByPlaceholder(COMMAND_MENU_PLACEHOLDER))
      .not.toBeInTheDocument()
  })
